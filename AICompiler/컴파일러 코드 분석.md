...
- 컴파일 옵션은 llvm::cl::opt를 이용
	![compile option 예시](https://imgs.hwan123.synology.me:19999/AICompiler/compile option 예시.png)
...
- 1) 아래와 같이 초기 부분에서는 session에서 invocation으로 필요한 정보들을 불러옴
  ...
  ![compile_lamda_invocation_init](https://imgs.hwan123.synology.me:19999/AICompiler/compile_lamda_invocation_init.png)
- 2) compile mode에 따라 Invocation Pipeline을 수행 -> 내부함수 [[runPipeline 분석]]
  ![compile_lamda_invocation_pipeline](https://imgs.hwan123.synology.me:19999/AICompiler/compile_lamda_invocation_pipeline.png)
- 3) output format에 따라 session state에 output을 저장
  ![compile_lamda_save_output](https://imgs.hwan123.synology.me:19999/AICompiler/compile_lamda_save_output.png)

# 5. 컴파일 실행
- 4번에서 작성한 람다함수(processBuffer)로 컴파일 수행, output에 저장 (output 포인터를 파일포인터를 주었으므로 파일에 저장됨)
  ![Pasted image 20260108112005](https://imgs.hwan123.synology.me:19999/AICompiler/Pasted image 20260108112005.png)
