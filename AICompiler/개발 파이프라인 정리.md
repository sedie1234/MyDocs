...
- 260316 기록 -> Claude Code를 이용한 개발 파이프라인 추가 (6장)
  - mlir-analyzer, ir-designer, dialect-doc-writer agent 추가
  - 7단계 개발 루프 정의 (분석→설계→문서화→구현→빌드→검증→반복)
  - 각 단계 사이 사용자 검토/확정 필수
...
# 6. Claude Code를 이용한 개발 파이프라인

- Dialect/Pass 개발 시 Claude Code의 Agent Team을 활용한 표준 작업 흐름
- 개발이 아닌 업무(문서 검토, 조사, 보고서 작성 등)는 이 파이프라인을 따르지 않아도 됨
...
│  2. 설계 제안                                           │
│     담당: ir-designer                                    │
│     - 분석 결과 기반 필요한 Op/Pass 리스트업             │
│     - 우선순위, 영향 범위, 난이도 포함 제안              │
│     - agent는 제안만, 최종 결정은 사용자                 │
│     - 결과: 설계 제안서                                  │
│                                                         │
│              ↓ 사용자 확정                                │
│                                                         │
│  3. 문서화                                              │
│     담당: dialect-doc-writer                             │
│     - 확정된 Op/Pass 설계를 docs에 문서화               │
│     - 작성 전 사용자와 내용 합의 (초안 → 피드백 → 승인) │
│     - 승인 후에만 실제 파일 작성                         │
│     - 대상: docs/MyDocs/AICompiler/                     │
│                                                         │
│              ↓ 사용자 승인                                │
│                                                         │
│  4. 코드 구현                                           │
│     담당: leader → mlir-developer + build-developer      │
│     - .td (TableGen), .cpp (Pass), .h (헤더)            │
│     - CMakeLists.txt                                     │
│     - docs의 설계 문서를 기준으로 구현                   │
│                                                         │
│              ↓                                            │
│                                                         │
│  5. 빌드                                                │
│     담당: leader → build-developer + bug-fixer           │
│     - 빌드 시도 및 오류 수정                             │
│                                                         │
│              ↓                                            │
│                                                         │
│  6. 검증                                                │
│     담당: leader → validator                             │
│     - iree-opt로 개별 Pass 적용하여 변환 결과 확인       │
│     - 검증 보고서 작성                                   │
│                                                         │
│              ↓ 사용자 검토                                │
│                                                         │
│  7. 1~6 반복                                            │
│     - 검증 결과에 따라 추가 분석/설계/구현 반복          │
│                                                         │
└─────────────────────────────────────────────────────────┘
```
...
| 항목 | 내용 |
|---|---|
| 담당 agent | `mlir-analyzer` |
| 호출 방법 | `mlir-analyzer로 [파일 경로] 분석해줘` |
| 입력 | 대상 MLIR 파일 (dispatch-level IR 등) |
| 출력 | 패턴 분석 보고서 (`reports/YYYYMMDD_*_패턴_분석.md`) |
| 다음 단계로 | 사용자가 보고서를 검토한 후 2단계 진행 요청 |

분석 내용:
- 모든 linalg.generic의 body 분석 및 패턴 분류
- Named ops (depthwise, batch_matmul, pooling, softmax 등) 카탈로그
- 구조 ops (padding, split, concat, reshape, transpose) 정리
- 전체 모델 구조 흐름 추론
- 기존 Pass 적용 시 변환/미변환 예측

### 6.2.2 설계 제안 (2단계)

| 항목 | 내용 |
|---|---|
| 담당 agent | `ir-designer` |
| 호출 방법 | `ir-designer로 설계 제안해줘` |
| 입력 | 1단계 분석 보고서 + 사용자 요구사항 |
| 출력 | Op/Pass 설계 제안서 |
| 다음 단계로 | 사용자가 제안을 검토하고 확정 |

핵심 원칙:
- **agent는 제안만 한다** — 최종 결정은 사용자
- 근거 제시 필수 (미변환 수량, 연쇄 영향, 난이도)
- 사용자 질문 시 대안 포함 답변
- 확정 전까지 "제안" 표현 사용
...
| 항목 | 내용 |
|---|---|
| 담당 agent | `dialect-doc-writer` |
| 호출 방법 | `dialect-doc-writer로 [확정 설계] 문서화해줘` |
| 입력 | 2단계에서 사용자가 확정한 설계 |
| 출력 | docs/MyDocs/AICompiler/ 내 문서 |
| 다음 단계로 | 사용자가 문서를 최종 확인한 후 4단계 진행 |

핵심 원칙:
- **문서 작성 전 반드시 사용자와 내용 합의**
- 초안을 텍스트로 보여주고 피드백 수렴
- 사용자 명시적 승인 후에만 파일 작성
- 기존 문서 스타일 준수

### 6.2.4 구현 → 빌드 → 검증 (4~6단계)

| 항목 | 내용 |
|---|---|
| 총괄 | `leader` |
| 호출 방법 | `leader에게 [작업] 요청해줘` |
| 구현 | `mlir-developer` (.td, .cpp, .h) + `build-developer` (CMake) |
| 빌드 | `build-developer` + `bug-fixer` (오류 수정) |
| 검증 | `validator` (iree-opt 개별 Pass 적용) |

- 코드 구현은 5장의 방법론(5.2 Dialect 구현, 5.3 Pass 구현)을 따름
- 검증은 5.5절의 iree-opt 방식을 따름

## 6.3 Agent 역할 매핑

| 파이프라인 단계 | Agent | 사용자 개입 |
|---|---|---|
| 1. MLIR 분석 | `mlir-analyzer` | 결과 검토 |
| 2. 설계 제안 | `ir-designer` | **확정** |
| 3. 문서화 | `dialect-doc-writer` | **합의 후 승인** |
| 4. 코드 구현 | `leader` → `mlir-developer` + `build-developer` | 필요 시 |
| 5. 빌드 | `leader` → `build-developer` + `bug-fixer` | 필요 시 |
| 6. 검증 | `leader` → `validator` | 결과 검토 |

## 6.4 파이프라인 외 Agent

| Agent | 역할 | 사용 시점 |
|---|---|---|
| `advisor` | 설계 조언, 기술 질의 | 언제든지 |
| `doc-reviewer` | 기존 문서 검토 | 언제든지 |
| `writer` | 일반 문서/보고서 작성 | 언제든지 |

## 6.5 호출 예시

```
# 1단계: MLIR 분석
mlir-analyzer로 pubH/test/yolov10_qdq.mlir 분석해줘

# 2단계: 설계 제안
ir-designer로 분석 보고서 기반 설계 제안해줘

# 3단계: 문서화
dialect-doc-writer로 확정된 설계 문서화해줘

# 4~6단계: 구현/빌드/검증
leader에게 IREEToQConv1x1Pattern 구현 요청해줘

# 파이프라인 외
advisor에게 depthwise conv ZP correction 방식 물어봐줘
```
